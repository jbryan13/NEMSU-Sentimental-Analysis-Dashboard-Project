...
$.getJSON(`process.php?alltime=`, function (data) {
  alltimeresponse = data.sort(
...
function chartpromoter(search, stringObj) {
  ...
  if (stringObj === "netpromoterchart") {
    ...
    charttype = "line";
    $.getJSON(`process.php?responsebyyear=${search}`, function (data) {
      for (let i = 0; i < data.length; i++) {
        if (data[i].length > 0) {
          for (let j = 0; j < data[i].length; j++) {
            if (i === 0) {
              dataArr[Number(data[i][j].timestamp.split("/")[1]) - 1]++;
            } else {
              dataArr[Number(data[i][j].date_of_feedback.split("/")[0]) - 1]++;
            }
          }
        }
      }
...
function servicesratings(year) {
  $.getJSON(`process.php?ratings=${year}`, function (data) {
    if (data.length === 0) {
...
function ratingtype(year) {
  ...
  var respondenttypes = {
    ...
    labelsrespondent,
    datarespondent;
  $.getJSON(`process.php?responsebyyear=${year}`, function (data) {
    let total = 0;
    for (let i = 0; i < data.length; i++) {
      if (data[i].length > 0) {
        for (let j = 0; j < data[i].length; j++) {
...
function div2() {
  let year = $('#piechartyear').val();
  let filteredcomment = $('#analyzesenti').val();
  $.getJSON(`process.php?comments=${year}&sentiment=${filteredcomment}`, function (data) {
    if  (data.length === 0){
